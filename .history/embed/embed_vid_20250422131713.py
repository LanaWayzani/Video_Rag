...
def embed_text_chunks(segments):
    print("Embedding transcript segments using NV-Embed-v2...")

    tokenizer = AutoTokenizer.from_pretrained(TEXT_MODEL_NAME, trust_remote_code=True)
    model = AutoModelForTextEncoding.from_pretrained(TEXT_MODEL_NAME, trust_remote_code=True).to(DEVICE).eval()
    ...
    for seg in tqdm(segments):
        text = seg["text"].strip()
        prompt = f"query: {text}"  # NV-Embed expects `query:` prompt
        inputs = tokenizer(prompt, return_tensors="pt", padding=True, truncation=True).to(DEVICE)

        with torch.no_grad():
            outputs = model(**inputs)
            emb = outputs.embedding  # NV-Embed returns embedding directly here
