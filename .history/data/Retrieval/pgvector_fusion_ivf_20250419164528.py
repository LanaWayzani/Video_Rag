...
if __name__ == "__main__":
    ...
    print(f"\n Top Transcript Match (regardless of score):")
    print(f"[{text_result[1]:.2f}s] {text_result[0]}  [score={text_score:.4f}]")

    print(f"\n Top Image Match (regardless of score):")
...
def run_pgvector_ivfflat_fusion(query, k=3):
    text_vec = embed_text(query)
    image_vec = embed_image_query(query)

    text_result = search_text_pgvector(text_vec, k=1)
    image_result = search_image_pgvector(image_vec, k=k)

    transcript_result = [(text_result[1], text_result[0], 1 - text_result[2])] if text_result else []
    image_results = [(img[0], img[1], 1 - img[2]) for img in [image_result] if image_result]

    return transcript_result, image_results
