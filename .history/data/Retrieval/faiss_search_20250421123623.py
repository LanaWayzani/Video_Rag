...
if __name__ == "__main__":
    ...
    if timestamp is not None:
        ...
        best_text_score = text_results[0][1] if text_results else 0.0

        if best_text_score < SIMILARITY_THRESHOLD or not keyword_in_transcript(user_query, text_results[0][0]["text"]):
        ...
        best_image_score = image_results[0][1] if image_results else 0.0
...
# ----------------------------
# Programmatic Entry for Streamlit or Evaluation
# ----------------------------
def run_retrieval(query, k=5):
    ...
    if best_text_score < SIMILARITY_THRESHOLD or not keyword_in_transcript(query, text_matches[0][0]["text"]):
        transcript_results = []
    else:
