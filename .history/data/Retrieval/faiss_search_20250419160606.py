...
if __name__ == "__main__":
    ...
    if timestamp is not None:
        ...
        if text_results[0][1] < SIMILARITY_THRESHOLD:
            print("\n⚠️ No confident transcript match found.")
        else:
            print("\n--- Transcript Matches ---")
...
def run_retrieval(query, k=5):
    ...
    q_vec = embed_query(query, model, tokenizer)
    text_matches = search_transcripts(q_vec, k)
    image_matches = search_keyframes(q_vec, k)

    transcript_results = [(seg["start"], seg["text"], score) for seg, score in text_matches]
    image_results = [(frame["time"], frame["image_path"], score) for frame, score in image_matches]
