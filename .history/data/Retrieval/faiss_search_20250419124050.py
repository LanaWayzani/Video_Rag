...
if __name__ == "__main__":
    ...
    if timestamp is not None:
        ...
        # Keyframe search
        image_results = search_keyframes(query_vec)
        if image_results[0][1] < SIMILARITY_THRESHOLD:
            print("\n⚠️ No confident keyframe match found.")
        else:
            print("\n--- Keyframe Matches ---")
            for frame, score in image_results:
                print(f"[{frame['time']:.2f}s] {frame['image_path']}  [score={score:.4f}]")
