...
query_type = st.sidebar.selectbox(
    "Choose query type:", ["Text", "Multimodal"]
)
...
if query_input:
    ...
    if not transcript_results or (query_type == "Multimodal" and backend != "Lexical (TF-IDF / BM25)" and not image_results):
        st.warning("No relevant match is found. Try rephrasing your question!")
        st.stop()
    ...
    if os.path.exists(video_path):
        st.video(video_path)
        st.caption(f"Jump to around {transcript_results[0][0]:.2f}s")
