...
def get_top1_timestamp(query, method: str, top_k: int = 5) -> float | None:
    """
    Run retrieval and return the top-1 timestamp (float) if results exist, else None.
    """
    if method == "faiss":
        transcript_results, _ = run_retrieval(query, top_k)
    elif method == "pgvector_ivfflat":
        transcript_results, _ = run_pgvector_ivfflat_fusion(query, top_k)
    elif method == "pgvector_hnsw":
        transcript_results, _ = run_pgvector_hnsw_fusion(query, top_k)
    elif method == "lexical":
        transcript_results, _ = run_lexical(query, top_k)
    else:
        raise ValueError(f"Unknown method: {method}")

    return transcript_results[0][0] if transcript_results else None
...
unanswerable = [
    "sed in the seminarWhat is the time complexity of the token sliding algorithm propo?",
