...
# ----------------------------
# Keyword Match Utility
# ----------------------------
def keyword_in_transcript(query, transcript_text, min_overlap=1):
...
if query_input:
    ...
    # Rejection logic
    reject_transcript = (
        not transcript_results or
        transcript_results[0][2] < 0.3 or
        not keyword_in_transcript(query_input, transcript_results[0][1])
    )

    reject_image = (
        query_type == "Multimodal" and
        backend != "Lexical (TF-IDF / BM25)" and
        (not image_results or image_results[0][2] < 0.3)
    )

    if reject_transcript and (query_type == "Text" or reject_image):
        st.warning("No relevant match found. Try rephrasing your question.")
        st.stop()
    ...
    if os.path.exists(video_path):
        ...
        st.markdown(f"Kindly refer to **{formatted_time}** in the video for the best match.")
    else:
