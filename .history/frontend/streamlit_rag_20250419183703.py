    ...
    if query_type == "Multimodal" and backend != "Lexical (TF-IDF / BM25)":
        ...
        for ts, img_path, score in image_results:
            ...
            if os.path.exists(full_path):
                st.image(full_path, caption=f"{ts:.2f}s | score={score:.4f}", use_column_width=True)st.image(full_path, caption=f"{ts:.2f}s | score={score:.4f}", use_container_width=True)
